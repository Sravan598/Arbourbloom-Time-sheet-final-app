"""
Generate BRD (Business Requirements Document) for AurborBloom
...
def add_table(doc, headers, rows, col_widths=None, header_fill="000000"):
...
def add_screenshot(doc, path, label, caption, width_inches=6.2):
    ...
    set_cell_border(cell, color="000000", sz="4")
    # Image
...
def add_portal_appendix(doc):
    ...
    add_para(doc,
        "This appendix presents annotated screen captures from a live tenant of the AurborBloom "
        "HRMS portal. Screens are grouped by role to illustrate how the platform satisfies the requirements stated "
        "earlier in this document. All screens are rendered on the aurborbloom tenant with full brand styling "
        "(logo, primary palette Black #000000 / Gold #D4AF37) applied automatically.")
...
def add_section_callout(doc, text, fill="FEF3C7"):
...
def setup_header_footer(doc, title):
    ...
    fr = fp.add_run("AurborBloom HRMS  |  © 2026  |  Page ")
    fr.font.size = Pt(8)
...
def build_brd():
    ...
    add_para(doc,
        ...
        "AurborBloom HRMS deployment. It serves as the contractual foundation upon which the "
    ...
    # 3. Company Overview
    add_heading(doc, "3. Company Overview — AurborBloom", level=1)
    ...
    add_para(doc,
        "AurborBloom positions itself as a high-velocity IT talent firm. The HR function is therefore not a back-office "
    ...
    add_screenshot(doc, f"{SCREENSHOTS_DIR}/01_tenant_landing.png",
                   "Live Capture — AurborBloom Tenant Landing (hr.aurborbloom.com)",
    ...
    add_para(doc,
        ...
        "impacted by the AurborBloom HRMS deployment.")
    ...
    add_bullets(doc, [
        ...
        "White-label branding: logo, primary/secondary colors, custom domain (hr.aurborbloom.com), email sender identity, browser tab title.",
    ...
    add_numbered(doc, [
        "AurborBloom will provide a single point of contact (SPOC) empowered to make functional decisions.",
        "AurborBloom will provide cleansed master data (employees, departments, managers) prior to migration.",
        "DNS access for hr.aurborbloom.com will be granted for CNAME verification.",
    ...
    brs = [
        ("BR-001", "Branding", "The platform must display AurborBloom branding (logo, colors, tab title, custom domain) consistently across every page and outbound email.", "Must", "BG-07"),
        ...
        ("BR-029", "Tenant Isolation", "No data of any other tenant shall be accessible from the aurborbloom tenant.", "Must", "BG-06"),
    ...
    add_table(doc, ["ID", "Risk", "L", "I", "Score", "Mitigation", "Owner"],
        [
            ...
            ["R-03", "DNS / CNAME setup delays custom domain go-live", 2, 3, 6,
             "Pre-validate DNS access early; fallback to *.hr.aurborbloom.com.", "IT"],
    ...
    add_para(doc,
        ...
        "needs of AurborBloom for the AurborBloom HRMS implementation. Material changes to this document shall "
    ...
    add_table(doc, ["Role", "Name", "Signature", "Date"],
        [
            ...
            ["Business Owner", "Head of Human Resources — AurborBloom", "_____________________", "__________"],
    ...
    out = f"{OUT_DIR}/AurborBloom_BRD_v1.0.docx"
