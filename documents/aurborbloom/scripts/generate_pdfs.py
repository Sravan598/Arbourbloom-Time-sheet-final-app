"""
Generate BRD and FRD PDFs for AurborBloom using ReportLab Platypus.
...
OUT = "/app/documents/aurborbloom"
ASSETS = f"{OUT}/assets"
LOGO = f"{ASSETS}/logo.png"

PRIMARY = HexColor("#000000")
SECONDARY = HexColor("#D4AF37")
...
def header_footer(canvas_obj, doc):
    ...
    canvas_obj.drawString(0.5 * inch, 0.2 * inch,
                          "AurborBloom HRMS  |  © 2026  |  Confidential")
...
PORTAL_SHOTS = [
    ("01_tenant_landing.png", "Screen A1 — Tenant Landing",
     "Branded landing page for hr.aurborbloom.com — logo, brand colors, and tagline applied automatically by the multi-tenant theming engine."),
    ("02_login.png", "Screen A2 — Login",
     "Login experience inherits the tenant brand. Tenant slug captured implicitly; only AurborBloom credentials are accepted."),
...
def portal_appendix(appendix_letter="A"):
    ...
    story.append(hr())
    story.append(Paragraph(
        "This appendix presents annotated screen captures from a live tenant of the "
        "AurborBloom HRMS portal. Screens are grouped by role to illustrate how the platform satisfies the "
        "requirements stated earlier in this document. All screens are rendered on the <b>aurborbloom</b> "
        "tenant with full brand styling (logo, primary palette <b>Black #000000 / Gold #D4AF37</b>) applied automatically.",
        S['body']))
...
def cover(doc_type, doc_code, version):
    ...
    elems.append(Paragraph("AURBORBLOOM", S['brand']))
    elems.append(Paragraph("Where People Bloom", S['tagline']))
...
def brd_story():
    story = []
    story += cover("Business Requirements Document", "AB-BRD-2026-001", "1.0")
    ...
    story.append(styled_table(
        ...
        [
            ["0.1", "2026-01-15", "Project Manager", "AurborBloom People Ops", "Initial draft"],
    ...
    story.append(styled_table(
        ...
        [
            ["Director, IT Services", "AurborBloom", "Approver"],
            ["Head of Human Resources", "AurborBloom", "Approver"],
            ["Chief Financial Officer", "AurborBloom", "Reviewer"],
            ["VP Engineering", "AurborBloom", "Reviewer"],
            ["Implementation Lead", "AurborBloom Engineering", "Owner"],
            ["Product Manager", "AurborBloom Engineering", "Author"],
        ],
    ...
    story.append(hr())
    story.append(Paragraph(
        "AurborBloom is a mid-sized IT Services and Software Development firm headquartered in the United States, "
    ...
    story.append(Paragraph(
        ...
        "the AurborBloom HRMS deployment. It serves as the contractual foundation upon which "
    ...
    # 3. Company Overview
    story.append(Paragraph("3. Company Overview — AurborBloom", S['h1']))
    story.append(hr())
    story.append(styled_table(
        ["Attribute", "Value"],
        [
            ["Legal Name", "AurborBloom"],
            ...
            ["Brand Tagline", "Where People Bloom"],
            ["Primary Brand Color", "#000000 (Deep Corporate Blue)"],
            ["Secondary Brand Color", "#D4AF37 (Action Blue)"],
            ["Public Website", "www.perfectgroupus.com"],
            ["HRMS Tenant Slug", "aurborbloom"],
            ["Custom Domain", "hr.aurborbloom.com"],
    ...
    story.append(Paragraph(
        "AurborBloom positions itself as a high-velocity IT talent firm. The HR function is therefore not a "
    ...
    story.append(figure(f"{ASSETS}/modules.png",
                        caption="Figure 5 — HRMS Module Map for AurborBloom"))
    story.append(PageBreak())
    ...
    story.append(Paragraph(
        ...
        "impacted by the AurborBloom HRMS deployment.", S['body']))
    story.append(styled_table(
        ...
        [
            ...
            ["VP Engineering", "Power User", "Medium", "High", "Pilot user, feedback loop"],
            ["HR Operations Lead", "Day-to-day Owner", "Medium", "High", "UAT, training champion"],
            ["Chief Information Security Officer", "Risk Approver", "High", "Medium", "Security review at each phase"],
    ...
    personas = [
        ...
        ("Sophia — Chief Information Security Officer",
    ...
    story.append(hr())
    story.append(Paragraph(
        "The following SMART business objectives have been agreed by the AurborBloom steering committee. Each "
    ...
    story += bullets([
        "Multi-tenant deployment of AurborBloom HRMS for the aurborbloom tenant.",
    ...
    story += numbered([
        "AurborBloom provides a single, empowered point of contact (SPOC).",
        "Cleansed master data (employees, departments, managers) provided prior to migration.",
        "DNS access for hr.aurborbloom.com granted for CNAME verification.",
    ...
    story.append(Paragraph(
        ...
        "business goal. Functional decomposition is in the FRD (AB-FRD-2026-001).", S['body']))
    brs = [
        ("BR-001", "Branding", "Display AurborBloom branding consistently across pages and outbound email.", "Must", "BG-07"),
        ...
        ("BR-029", "Tenant Isolation", "No cross-tenant data accessible from aurborbloom.", "Must", "BG-06"),
    ...
    story.append(styled_table(
        ...
        [
            ...
            ["R-03", "DNS / CNAME setup delays go-live", "2", "3", "6",
             "Pre-validate DNS; fallback to *.hr.aurborbloom.com.", "IT"],
    ...
    story.append(Paragraph(
        ...
        "needs of AurborBloom for the AurborBloom HRMS implementation. Material changes shall be processed "
        "via formal change control.", S['body']))
    story.append(Spacer(1, 8))
    story.append(styled_table(
        ["Role", "Name", "Signature", "Date"],
        [
            ["Executive Sponsor", "Chief Technology Officer — AurborBloom", "_____________________", "__________"],
            ["Business Owner", "Head of People Operations — AurborBloom", "_____________________", "__________"],
            ["Financial Approver", "Chief Financial Officer — AurborBloom", "_____________________", "__________"],
...
def frd_story():
    story = []
    story += cover("Functional Requirements Document", "AB-FRD-2026-001", "1.0")
    story.append(NextPageTemplate('Body'))
    ...
    story.append(Paragraph(
        ...
        "discrete, testable functional specifications for the AurborBloom HRMS platform deployed for AurborBloom "
    ...
    story.append(hr())
    story.append(Paragraph(
        "The AurborBloom HRMS is a multi-tenant SaaS platform that comprising a React single-page application, a FastAPI backend, and "
        "a MongoDB persistence tier on Kubernetes. For AurborBloom the platform is delivered as a fully "
        "white-labeled tenant identified by the slug 'aurborbloom', accessible at hr.aurborbloom.com.",
        S['body']))
    ...
    story += bullets([
        "Tenant resolution via slug path or custom domain (hr.aurborbloom.com).",
    ...
    story.append(hr())
    story.append(Paragraph(
        "Roles below are scoped to the AurborBloom tenant. Super Admin is platform-level and reserved for "
    ...
    story.append(styled_table(
        ...
        [
            ...
            ["FR-008", "Display AurborBloom branding on all auth pages.", "Must", "BR-001"],
        ],
    ...
    story.append(styled_table(
        ...
        [
            ...
            ["Main Flow",
             ...
             "2) System validates code against aurborbloom tenant.<br/>"
    ...
    stories_data = [
        ...
        ["US-04", "As an Admin, I want browser tab titles to display 'AurborBloom — HR Portal' so brand identity is preserved.",
...
def main():
    brd_path = f"{OUT}/AurborBloom_BRD_v1.0.pdf"
    doc = build_doc(brd_path, "AurborBloom HRMS • Business Requirements Document")
    doc.build(brd_story())
    print(f"BRD PDF saved → {brd_path}")

    frd_path = f"{OUT}/AurborBloom_FRD_v1.0.pdf"
    doc = build_doc(frd_path, "AurborBloom HRMS • Functional Requirements Document")
    doc.build(frd_story())
